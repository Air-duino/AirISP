...
namespace AirISP
{
    static class EraseFlashClass
    {
        ...
        {
            ColorfulConsole.InfoLine(Tool.IsZh() ? "擦除flash中（请耐心等待）..." : "Erasing flash (this may take a while)...");
            ...
            {
                ColorfulConsole.SuccessLine(Tool.IsZh() ?
            ...
            {
                ColorfulConsole.WarnLine(Tool.IsZh() ? "擦除失败，请重试" : "Erase failed, please retry later");
                return false;
            }
        }
    }
